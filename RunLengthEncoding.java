...
public class RunLengthEncoding implements Iterable {
    ...
    public RunLengthEncoding(int width, int height) {
        ...
        this.height = height;
        DList1 d = new DList1();
        d.insertBack((short) 0, (short) 0, (short) 0, width * height);
    }
    ...
    int[] blue, int[] runLengths) {
        ...
        for (int i = 0; i <= runLengths.length - 1; i++)
        {
            d.insertBack((short) red[i], (short) green[i], (short) blue[i], runLengths[i]);   
        }
    }
    ...
    public RunIterator iterator() {
        RunIterator x = new RunIterator(d);
        return x;
    }
        ...
        if (x.hasNext())
        {
            ...
            for (int i = 0; i < height; i++)
            {
                for (int j = 0; j < width; j++)
                {
                    if ((x.hasNext()) && (ar[0]==0)) {
                        ar = x.next();
                        pic.setPixel(j, i, (short) ar[1], (short) ar[2], (short) ar[3]);
                        --ar[0];
                    }
                    else { 
                        pic.setPixel(j, i, (short) ar[1], (short) ar[2], (short) ar[3]);
    ...
    public RunLengthEncoding(PixImage image) {
        ...
        DList1 newd = new DList1();      
        width = image.getWidth();
        height = image.getHeight();
        d = new DList1();
        DListNode1 d1= new DListNode1();    
        DListNode1 d2= new DListNode1();  
        
        for (int i = 0; i < image.getHeight(); i++)
        { 
            for (int j = 0; j < image.getWidth(); j++)
            {  
                newd.insertBack(image.getRed(j, i), image.getGreen(j, i), image.getBlue(j, i), 1);
            }
        }
    ...
    public void compress (DList1 listz)
    {
        DListNode1 list1 = new DListNode1();    
        DListNode1 list2 = new DListNode1(); 
        list1 = listz.head.prev;
        list2 = listz.head;
        d = new DList1();

        for (int i = 0; i <listz.size; i++)
        { 
            if (list1 != listz.head.prev)
            {
                if(list1.red == list2.red && list1.green == list2.green && list1.blue == list2.blue)
                {
                    d.tail.run++;
                }
                else
                {
                    d.insertBack(list2.red, list2.green, list2.blue, 1);
                }
                list1 = list2;
                list2 = list2.next;
            }
    ...
    public void check() {
        ...
        while (x.hasNext() == true)
        {
            ...
            {
                z = x.next();
                if (array[0]!=z[0] && array[1] == z[1] && array[2] == z[2] && array[3] == z[3])
                {System.out.println("Error");
                    return;}
                if (z[0] < 1)
                {System.out.println("Error");
    ...
    public void setPixel(int x, int y, short red, short green, short blue) {
        DListNode1 node = d.head;
        DList1 z = new DList1();
        int total = 0;

        while (node != null)
        {
            for (int i = 0; i < node.run; i++)
            { 
                z.insertBack(node.red, node.green, node.blue, 1);
            }
            node = node.next;
        }
